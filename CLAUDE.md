...
- A commit marks a finished TODO item: code, tests, README, CHANGELOG, `config.example.yaml`,
  ...
  `TODO.md`. Commits stay local until the maintainer says push.
...
- CI is one Ubuntu job per push; minutes are billed. Never add a runner, trigger or job without
  stating what it costs per push.
